...
def train_one_epoch(
    ...
    num_updates = epoch * len(loader)/args.grad_accu
    loss_value = 0
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if loss_scaler is not None:
            ...
            if (batch_idx + 1) % args.grad_accu == 0:
                optimizer.step()
        if (batch_idx+1) % args.grad_accu==0:
            optimizer.zero_grad()
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                reduced_loss = reduce_tensor(loss.data*args.grad_accu, args.world_size)
                losses_m.update(reduced_loss.item()*args.grad_accu, input.size(0))
